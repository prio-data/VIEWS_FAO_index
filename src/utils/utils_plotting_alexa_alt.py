...
def plot_country_time_series(df, country_ids, feature, time_periods=None, figsize=(12, 8), PATH=PATH, logo_placement = (0.9, 0.85), legend_placement=(0.8, 1), force_color = None, custom_title = None, custom_ylabel = None, save_plot = False, PATH_PLOT = None):
    """
   ...
   # 
    ...
    for idx, country_id in enumerate(country_ids):
        ...
        # Set the plot title based on the custom_title parameter

        # Debug print statement to check custom_title
        print(f"Setting title: {custom_title}")

        # Set the plot title based on the custom_title parameter
        if custom_title is not None:
            plt.title(str(custom_title), fontsize=16, fontname='DejaVu Sans')
        else:
    ...
    # Set the ylabel based on the custom_ylabel parameter
...
def plot_country_period_map(df, country_id, feature, periods, figsize=(16, 8), marker_size=64, PATH=PATH, save_plot = False):
...
def plot_country_period_map_alexa_alt(df, country_id, feature, periods, figsize=(16, 8), marker_size=64, PATH='', save_plot=False):
    ...
    # Get the min and max values of the feature for that country over all the periods
    unique_values = df[(df['c_id'] == country_id) & (df[time_period].isin(periods))][feature].unique()

    # Check if all values are the same
    if len(unique_values) == 1:
        # If all values are the same, create a small range around that value
        vmin = unique_values[0] - 0.001
        vmax = unique_values[0] + 0.001
    else:
        vmin = df[(df['c_id'] == country_id) & (df[time_period].isin(periods))][feature].min()
        vmax = df[(df['c_id'] == country_id) & (df[time_period].isin(periods))][feature].max()
    ...
    for idx, periode in enumerate(periods):
        ...
        # create the scatter
        cmap = 'rainbow_r' if 'likelihood' in feature else 'rainbow'
        sc1 = axes[idx].scatter(sub_df['col'], 
                        sub_df['row'], 
                        c=sub_df[feature], 
                        cmap=cmap, 
                        marker='s', 
